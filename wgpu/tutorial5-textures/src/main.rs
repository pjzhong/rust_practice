...
struct State {
    ...
    saber_bind_group: wgpu::BindGroup,
    challenge: bool,
}
...
impl State {
    async fn new(window: &Window) -> Self {
        ...
        let saber_bytes = include_bytes!("saber.png");
        //This texture struct has no method to interact with the data directly
        let diffuse_texture =
            texture::Texture::from_bytes(&device, &queue, diffuse_bytes, "happy-tree.png").unwrap();
        let saber_texture =
            texture::Texture::from_bytes(&device, &queue, saber_bytes, "saber.png").unwrap();
        ...
        let saber_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("saber_bind_group"),
            layout: &texture_bind_group_layout,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
                    resource: wgpu::BindingResource::TextureView(&saber_texture.view),
                },
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: wgpu::BindingResource::Sampler(&saber_texture.sampler),
                },
            ],
        });
        ...
        let challenge_vertices = (0..challenge_num_vertices)
            .map(|i| {
                ...
                Vertex {
                    ...
                    tex_color: [(1.0 + theta.cos()) / 2.0, (1.0 + theta.sin()) / 2.0],
                }
            })
            .collect::<Vec<_>>();
